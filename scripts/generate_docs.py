...
POSTMAN_OUTPUT_PATH = os.path.join(COLLECTIONS_DIR, "TearsDeepMind_FULL_API.postman_collection.json")
...
def build_postman_collection():
    ...
    collection = {
        "info": {"_postman_id": str(uuid.uuid4()), "name": "TearsDeepMind FULL API", "schema": "https://schema.getpostman.com/json/collection/v2.1.0/collection.json"},
        "item": [
            {
                "name": "V2 (Industrial Engine)",
                "item": [
                    {"name": "Start Industrial Job", "request": {"method": "POST", "url": "{{baseUrl}}/api/v2/crawler/jobs/DailyAnalysis/5"}},
                    {"name": "Check Report Existence (Deterministic)", "request": {"method": "GET", "url": "{{baseUrl}}/api/v2/crawler/check/DailyAnalysis/2026-01-23"}},
                    {"name": "Stream Job Events", "request": {"method": "GET", "url": "{{baseUrl}}/api/v2/crawler/stream/{{jobId}}"}}
                ]
            },
            {
                "name": "V1 (Standard & Legacy)",
                "item": [
                    {"name": "Sync Extract (Blocking)", "request": {"method": "GET", "url": "{{baseUrl}}/api/v1/crawler/extract/DailyAnalysis/2"}},
                    {"name": "Check Feed for New Content", "request": {"method": "GET", "url": "{{baseUrl}}/api/v1/crawler/check/DailyAnalysis"}},
                    {"name": "Start Legacy Async Job", "request": {"method": "POST", "url": "{{baseUrl}}/api/v1/crawler/async/extract/DailyAnalysis/5"}},
                    {"name": "Get Legacy Job Status", "request": {"method": "GET", "url": "{{baseUrl}}/api/v1/crawler/async/status/{{jobId}}"}}
                ]
            },
            {
                ...
                "item": [
                    {
                        "name": "Daily Analysis",
                        "item": [
                            {"name": "GET All Records", "request": {"method": "GET", "url": "{{baseUrl}}/api/history/daily-analysis"}},
                            {"name": "GET Record by Date", "request": {"method": "GET", "url": "{{baseUrl}}/api/history/daily-analysis/2026-01-23"}},
                            {
                            ...
                            {"name": "DELETE Record", "request": {"method": "DELETE", "url": "{{baseUrl}}/api/history/daily-analysis/2026-01-23"}}
                        ]
                    },
                    {
                        "name": "Quant Memory",
                        "item": [
                            {"name": "GET All Records", "request": {"method": "GET", "url": "{{baseUrl}}/api/history/quant-memory"}},
                            {"name": "GET Record by Date", "request": {"method": "GET", "url": "{{baseUrl}}/api/history/quant-memory/2026-01-23"}},
                            {
                                "name": "CREATE/UPDATE Record",
                                "request": {
                                    "method": "POST", "header": [{"key": "Content-Type", "value": "application/json"}],
                            ...
                            {"name": "DELETE Record", "request": {"method": "DELETE", "url": "{{baseUrl}}/api/history/quant-memory/2026-01-23"}}
                        ]
                    }
                ]
            }
        ],
        "variable": [{"key": "baseUrl", "value": "http://localhost:8080"}, {"key": "jobId", "value": "insert-job-uuid-here"}]
    }

    with open(POSTMAN_OUTPUT_PATH, 'w') as f:
        json.dump(collection, f, indent=2)
    print(f"✅ Colección Postman 1:1 generada en: {POSTMAN_OUTPUT_PATH}")
...
def build_drawio_diagram():
  ...
  <diagram name="TearsDeepMind Architecture">
    ...
    with open(DRAWIO_OUTPUT_PATH, 'w') as f:
        f.write(xml_content)
    print(f"✅ Diagrama Draw.io nativo regenerado en: {DRAWIO_OUTPUT_PATH}")
